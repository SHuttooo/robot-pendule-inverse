...
Le robot reconstruit dans MuJoCo depuis son assemblage SolidWorks, le double PID
du firmware porté tel quel comme témoin, et un agent PPO entraîné pour le
...
**`modeles/balancier.xml`** : le robot au format MJCF, le XML de MuJoCo. Il est
généré par `modele.py`, jamais édité à la main, et charge les maillages depuis
`hardware/cao/stl_assemblage/`. Ce fichier utilise l'actionneur en vitesse de
MuJoCo ; le modèle fidèle du moteur pas-à-pas, celui de l'entraînement, est
ajouté par `modele.xml('couple')` et `moteur.py`.

Il n'y a pas d'URDF : rien dans ce projet n'utilise ROS.
...
| `firmware.py` | le double PID du `.ino` transcrit en Python |
...
| `03_pid.py` | le double PID du firmware en simulation (`--vue`, `--silence`) |
...
| `08_agent.py` | regarder un agent (`--duel` contre le double PID, `--film`) |
...
| | ancien agent | **nouvel agent** | double PID |
|---|---|---|---|
...
Poussée isolée de 150 ms à 21 cm de l'axe (`17_pousser_agents.py`) : double PID
